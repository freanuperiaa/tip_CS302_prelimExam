...
#Simple moving average forecast - 3 year period
sma_three_year_period = np.zeros(55)
for index, value in enumerate(actual_data):
    if index in [0,1,2]:
        sma_three_year_period[index] = None
    else:
        sma_three_year_period[index] = (actual_data[index - 1] + actual_data[index-2] + actual_data[index-3])/3


#Simple moving average forecast - 5 year period
sma_five_year_period = np.zeros(55)
for index, value in enumerate(actual_data):
    if index in [0,1,2,3,4]:
        sma_five_year_period[index] = None
    else:
        sma_five_year_period[index] = (actual_data[index-1] + actual_data[index-2] + actual_data[index-3] + 
                actual_data[index-4] + actual_data[index-5])/5
...
plt.plot(years, sma_three_year_period, marker = 'o', label = "forecasted amount using thee-year period")
plt.plot(years, sma_five_year_period, marker = 'o',label = "forecasted amount using five-year period")
plt.xlabel('Year')
plt.ylabel('Carbon emission in kilotonnes(kt)')
plt.title('Forecast of Carbon emisssion of Philippines from 1960 to 2014 using Simple Moving Average Forecasting Technique')
plt.legend()
plt.show()
